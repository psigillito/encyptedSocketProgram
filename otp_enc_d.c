...
//Takes two buffers and outputs an encrypted string 
char* generateEncryptedData(char textData[], char keyData[]){
	...
	//For each character in textdata and key data find index in translate list, add together and %27
	for(i=0; i<(dataLength);i++){
		...
		//add together, %27, and append to encrypted data
		int newValue = (tempy + keyTemp)% 27;
	...
	while(1){
		...
		if( childProcessCount  < 5){
			...
			if( spawnPid == 0){
				...
				//ENCRYPT DATA
					char* encryptedResponse = generateEncryptedData(textBuffer, buffer);

				//SEND ENCRYPTED DATA

					//Buffer for sending data 
					char encryptedOutputBuffer[200000];
        				memset(encryptedOutputBuffer, '\0',200000);
        		
					//Copy Data into Buffer for ease of sending 
					int x = 0;
					int max = strlen(encryptedResponse);
					for(x = 0; x < max; x++){
						encryptedOutputBuffer[x]= encryptedResponse[x];
					}
					
					//Number of bytes needed to be sent 
					int BytesToSend = strlen(encryptedOutputBuffer)*sizeof(char);
       					int sentAmount=0;

					//keep sending until all sent
	       				while(sentAmount < BytesToSend){
						sentAmount += send(new_connection,encryptedOutputBuffer+sentAmount,199999,0);
       			 		}
